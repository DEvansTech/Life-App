    ...
    <SafeAreaView>
      ...
      <ScrollView>
        ...
        <View className="bg-white pl-6 pt-2">
          { items.map((item) => (
            <View key={item.id} className="flex-row py-2.5 border-b border-b-[#C9C9C9]">
              <Text className={cn("text-sm font-normal mr-auto", {
                ['text-[' + item.titleColor + ']']: item.titleColor,
                'text-black': !item.titleColor
              })}>{item.title}</Text>
              <Text className={cn("text-sm font-normal mr-4", {
                ['text-[' + item.descColor + ']']: item.descColor,
                'text-[#58575D]': !item.descColor
              })}>{item.value}</Text>
              <Text className="mr-4"><Ionicons name="chevron-forward" size={20} color="#AAA" /></Text>
            </View>
          ))}
        </View>
      </ScrollView>
