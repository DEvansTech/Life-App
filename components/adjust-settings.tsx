  ...
  return (
    <View className="w-full">
      {settingsList.map((setting) => {
        return (
          <View className="flex h-[42px] flex-row py-2.5 justify-between border-b border-stone-300">
            <Text className="text-black text-sm font-normal leading-snug">
              {setting.field}
            </Text>

            <TouchableOpacity className="flex flex-row items-center">
              <Text className={`pr-2 text-primary-color text-sm font-medium leading-snug ${setting.field === 'Phone Number' ? 'text-zinc-600' : 'text-primary-color'}`}>
                {setting.updateField}
              </Text>
